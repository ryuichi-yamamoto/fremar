...
mens_grandchild_array = [
                          ['シャツ','スウェット','ポロシャツ','Tシャツ/カットソー(七分/長袖)','Tシャツ/カットソー(半袖/袖なし)','カーディガン','パーカー','ジャージ','ベスト','その他'],
                          ...
                          ['スーツジャケット','スーツ','ベスト','スラックス','セットアップ','その他'],
                          ['キャップ','ハット','ニットキャップ/ビーニー','ハンチング/ベレー帽','キャスケット','サンバイザー','その他'],
                          ['ネックレス','ブレスレット','バングル/リストバンド','リングピアス(片耳用)','ピアス(両耳用)','アンクレット','その他'],
                          ['長財布','折り財布','マネークリップ','コインケース/小銭入れ','名刺入れ/定期入れ','キーケース','キーホルダー','ネクタイ','手袋','ハンカチ','ベルト','マフラー','ストール','バンダナ','ネックウォーマー','サスペンダー','ウォレットチェーン','サングラス/メガネ','モバイルケース/カバー手帳','ストラップ','ネクタイピン','カフリンクス','イヤマフラー','傘','レインコート','ミラー','タバコグッズ','その他'],
                          ['腕時計(アナログ)','腕時計(デジタル)','ラバーベルト','レザーベルト','金属ベルト','その他'],
...
baby_kids_grandchild_array = [
                              ['トップス','アウターパンツ','スカート','ワンピース','ベビードレス','おくるみ下着/肌着','パジャマ','ロンパース','その他'],
                              ['トップス','アウターパンツ','おくるみ','下着/肌着','パジャマ','ロンパース','その他'],
                              ['トップス','アウターパンツ','おくるみ','下着/肌着','パジャマ','ロンパース','その他'],
                              ...
                              ['コートジャケット/上着','トップス(Tシャツ/カットソー)','トップス(トレーナー)','トップス(その他)','パンツ','セットアップ','パジャマ','フォーマル/ドレス','和服','浴衣','甚平','水着','その他'],
                              ['コート','ジャケット/上着','トップス(Tシャツ/カットソー)','トップス(トレーナー)','トップス(その他)','ボトムス','パジャマ','その他'],
                              ['スニーカー','サンダル','ブーツ','長靴','その他'],
                              ['靴下/スパッツ','帽子','エプロン','サスペンダー','タイツ','ハンカチ','バンダナ','ベルト','マフラー','傘','手袋','スタイバッグ','その他'],
...
interior_grandchild_array = [
                              ...
                              ['布団/毛布','枕シーツ/カバー','その他'],
                              ['ごみ箱','ウェルカムボード','オルゴール','クッション','クッションカバー','スリッパ','ラック','ティッシュボックス','バスケット/かご','フォトフレーム','マガジンラック','モビール','花瓶','灰皿','傘立て','小物入れ','置時計','掛時計/柱時計鏡(立て掛け式)','鏡(壁掛け式)','置物風鈴植物/観葉植物','その他'],
                              ['正月','成人式','バレンタインデー','ひな祭り','子どもの日','母の日','父の日','サマーギフト/お中元夏/夏休み','ハロウィン','敬老の日','七五三','お歳暮','クリスマス','冬一般','その他'],
                              ['その他']
                            ]
...
toy_grandchild_array = [
                        ...
                        ['コミック/アニメ','特撮','ゲーム','キャラクター','SF/ファンタジー/ホラー','アメコミ','スポーツ','ミリタリー','その他'],
...
cosmetics_grandchild_array = [
                              ...
                              ['化粧水/ローション','乳液/ミルク','美容液','フェイスクリーム','洗顔料','クレンジング/メイク落とし','パック/フェイスマスク','ジェル/ゲルブースター/導入液','アイケア','リップケア','トライアルセット/サンプル','洗顔グッズ','その他'],
...
smartphone_child_array = ['スマートフォン/携帯電話','スマホアクセサリー','PC/タブレット','カメラ','テレビ/映像機器','オーディオ機器','美容/健康','冷暖房/空調','生活家電','その他']
smartphone_grandchild_array = [
                                ...
                                ['ポータブル','プレーヤー','イヤフォンヘッド','フォンアンプ','スピーカー','ケーブル/シールド','ラジオ','その他'],
                                ['ヘアドライヤー','ヘアアイロン','美容機器電気','シェーバー','電動歯ブラシ','その他'],
...
smartphone_child_array.each_with_index do |child, i|
  child = parent.children.create(name: child)
  smartphone_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
hand＿made_child_array = ['アクセサリー(女性用)','ファッション/小物','アクセサリー/時計','日用品/インテリア','趣味/おもちゃ','キッズ/ベビー','素材/材料','二次創作物','その他']
hand＿made_grandchild_array = [
...
hand＿made_child_array.each_with_index do |child, i|
  child = parent.children.create(name: child)
  hand＿made_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
